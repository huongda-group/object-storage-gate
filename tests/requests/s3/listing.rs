...
///
/// axum routes `/{bucket}` and `/{bucket}/` separately and loco refuses to register both, so this one is added straight on the router.
/// Without it the request matches nothing, falls through to the console, and a `ListObjectsV2` comes back as a page of HTML — which the conformance suite found and no Rust test did.
#[tokio::test]
#[serial]
